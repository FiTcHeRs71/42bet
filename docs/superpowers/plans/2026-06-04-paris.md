...
Exécution subagent-driven sur la branche `feat/paris`. Reprendre à la **Task 4**.
...
- [ ] **Task 4 — `actions.ts`** (server action `placeBet`) ← PROCHAINE (modèle standard/Sonnet : pièce d'intégration).
- [ ] **Task 5 — `bet-form.tsx`** (client component).
- [ ] **Task 6 — `match-row.tsx`** (affichage par état + session).
- [ ] **Task 7 — `page.tsx`** (câblage session + pronos).
- [ ] **Task 8 — gates finaux + merge `--no-ff` dans `main`**.

Fondations pures + I/O posées ; reste l'orchestration (Task 4) et l'UI (5-7).
