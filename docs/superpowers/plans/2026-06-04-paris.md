...
## État d'avancement (maj 2026-06-04)

Exécution sur la branche `feat/paris`. Code complet ; reste le **merge** (Task 8 §4).

- [x] **Task 1 — `bet-rules.ts`** (pur, 14 tests) — commits `1cd8814` + `1c90290` (fix qualité : `BetInput` retiré, test borne 99). Reviews spec ✅ + qualité ✅.
- [x] **Task 2 — `users.ts`** (`resolveUserId`) — commit `c34ef3a`. Vérif inline ✅ (+ `import "server-only"` ajouté).
- [x] **Task 3 — `bets.ts`** (`listMyBets` + `upsertBet`) — commit `f446e8b`. Vérif inline ✅.
- [x] **Task 4 — `actions.ts`** (server action `placeBet`) — commit `cc50101`. typecheck + lint ✅.
- [x] **Task 5 — `bet-form.tsx`** (client component) — commit `1635f91`.
- [x] **Task 6 — `match-row.tsx`** (affichage par état + session) — commit `650bfe5`.
- [x] **Task 7 — `page.tsx`** (câblage session + pronos) — commit `ff7b9da`. Build ✅ (`/matches` ƒ Dynamic).
- [ ] **Task 8 — gates finaux ✅ (55 tests, typecheck, lint, build, anti-secret) ; reste merge `--no-ff` dans `main`.**

Tout le code est posé et vérifié vert ; reste uniquement l'intégration dans `main`.
